...
@test "labeled switch - detects undefined when skipping definition state" {
    run compile_and_run "$TEST_CASES/undefined_safety/labeled_switch/undefined_skip_state.zig"
    [ "$status" -ne 0 ]
...
@test "labeled switch - all paths to final state define variable" {
    run compile_and_run "$TEST_CASES/undefined_safety/labeled_switch/all_paths_define.zig"
    [ "$status" -eq 0 ]
}

@test "labeled switch - detects undefined use during transition" {
    run compile_and_run "$TEST_CASES/undefined_safety/labeled_switch/use_in_transition.zig"
    [ "$status" -ne 0 ]
    [[ "$output" =~ "use of undefined value" ]]
...
@test "labeled switch - tokenizer pattern with multiple paths" {
    run compile_and_run "$TEST_CASES/undefined_safety/labeled_switch/tokenizer_pattern.zig"
    [ "$status" -ne 0 ]
    [[ "$output" =~ "use of undefined value" ]]
...
@test "labeled switch - variable defined then used across dispatch" {
    run compile_and_run "$TEST_CASES/undefined_safety/labeled_switch/defined_then_used_across_dispatch.zig"
    [ "$status" -eq 0 ]
}

@test "labeled switch - cyclic dispatch converges" {
    run compile_and_run "$TEST_CASES/undefined_safety/labeled_switch/cyclic_dispatch_converges.zig"
    [ "$status" -eq 0 ]
}
...
@test "labeled switch - allocate and free across states" {
    run compile_and_run "$TEST_CASES/allocator_safety/labeled_switch/alloc_free_states.zig"
    [ "$status" -eq 0 ]
}

@test "labeled switch - detects leak when skipping free state" {
    run compile_and_run "$TEST_CASES/allocator_safety/labeled_switch/leak_skip_free.zig"
    [ "$status" -ne 0 ]
    [[ "$output" =~ "memory leak" ]]
...
@test "labeled switch - detects double free on state revisit" {
    run compile_and_run "$TEST_CASES/allocator_safety/labeled_switch/double_free_revisit.zig"
    [ "$status" -ne 0 ]
    [[ "$output" =~ "double free" ]]
...
@test "labeled switch - detects double free across cases" {
    run compile_and_run "$TEST_CASES/allocator_safety/labeled_switch/double_free_across_cases.zig"
    [ "$status" -ne 0 ]
    [[ "$output" =~ "double free" ]]
...
@test "labeled switch - detects use after free via dispatch" {
    run compile_and_run "$TEST_CASES/allocator_safety/labeled_switch/use_after_free_via_dispatch.zig"
    [ "$status" -ne 0 ]
...
@test "labeled switch - detects leak via early exit" {
    run compile_and_run "$TEST_CASES/allocator_safety/labeled_switch/leak_via_early_exit.zig"
    [ "$status" -ne 0 ]
    [[ "$output" =~ "memory leak" ]]
...
@test "labeled switch - null check propagates across dispatch" {
    run compile_and_run "$TEST_CASES/null_safety/labeled_switch/null_check_across_dispatch.zig"
    [ "$status" -eq 0 ]
}

...
@test "detects free of global memory (laundered through function)" {
    run compile_and_run "$TEST_CASES/allocator_safety/basic/free_global_ptr_laundered.zig"
    [ "$status" -ne 0 ]
    [[ "$output" =~ "free of global/comptime memory" ]]
...
@test "detects free of global memory (indirect through function)" {
    run compile_and_run "$TEST_CASES/allocator_safety/basic/free_global_ptr_indirect.zig"
    [ "$status" -ne 0 ]
    [[ "$output" =~ "free of global/comptime memory" ]]
...
@test "detects free of global slice" {
    run compile_and_run "$TEST_CASES/allocator_safety/basic/free_global_slice.zig"
    [ "$status" -ne 0 ]
    [[ "$output" =~ "free of global/comptime memory" ]]
